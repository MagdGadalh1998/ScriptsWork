...
namespace TraceCurve
{
	public class TraceInput : MonoBehaviour
	{
		...
		public UnityEvent OnShapeFinish;
		#endregion
		...
		private void SetDot()
		{
			Vector3 newPosition = TracePainter.BrushObject.transform.position;
			Vector2 newPositionv2 = (Vector2)TracePainter.BrushObject.transform.position;
			if (!IsAllFiguresFinished && MoveDirection == Move.Forward)
			{
				print("Dot");
				//previousMousePosition = newPosition;
				isDot = true;
				TracePainter.AddToRenderQueue(new[] { newPositionv2, newPositionv2 });
				//TracePainter.UpdateOnce = true;
		...
		private bool TryMoveForward(Vector3 movingPosition, ref Vector3 position)
		{
            ...
            {
				distance = Vector2.Distance(nearestPoint, NextPoint);
			}
			//print(distance + " " + ConnectRadius);
			if (distance < ConnectRadius)
			{
				...
				if (IsFigureFinished)
				{
					...
					{
						print("finished");
						OnShapeFinish.Invoke();
						TracePainter.BrushObject.gameObject.SetActive(false);
						position = NextPoint;
						...
						//todo
						//GameManager.instance.onDrawComplete.Invoke();
						return false;
					}
