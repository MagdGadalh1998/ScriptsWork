...
namespace TraceCurve
{
	public class GeometryContainer : MonoBehaviour
	{
		...
		private void Awake()
		{
			Transform initialPosition = gameObject.GetComponent<TraceInput>().initialPosition;
			if (initialPosition != null)
			{

				StartCoroutine(ChangePosition(initialPosition));
			}
			UpdateSegmentsData = true;
			UpdateSegments();
		}

		IEnumerator ChangePosition(Transform initialPosition)
        {
			yield return new WaitForEndOfFrame();
			transform.parent.position = initialPosition.position;
			print(initialPosition.position);
			UpdateSegments();
